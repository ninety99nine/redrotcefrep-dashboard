    ...
    <div class="bg-white shadow-lg rounded-lg border space-y-3 p-4 mb-4">
        ...
        <div v-if="isLoadingOrder" class="flex items-center space-x-2">
            <ShineEffect v-if="isLoadingOrder">
                <RoundSkeleton size="w-8 h-8 bg-green-100"></RoundSkeleton>
            </ShineEffect>
            <p class="font-bold text-lg">Collection</p>
        </div>

        <div v-else-if="order.collectionVerified.status" class="flex items-center space-x-2 font-bold text-lg text-green-500">
        ...
        <template v-if="!isLoadingOrder && !order.collectionVerified.status">
            ...
            <p class="text-xs font-bold text-yellow-400">{{ order.collectionVerified.description }}</p>
        ...
        <div v-else-if="(order.collectionType || order.destinationName)" :class="['flex items-end space-x-4 text-sm', isLoadingOrder || (order && order.collectionVerified.status) ? 'border-b border-dashed pb-2' : 'border-t border-dashed pt-2 mt-2']">
        ...
        <div v-if="isLoadingOrder || (order && order.collectionVerified.status)" class="space-y-3">

            <!-- Collection Information -->
            <div class="space-y-2 border-b border-dashed pb-2 mb-2">
                ...
                <div class="flex items-center space-x-2 text-xs">
                    ...
                    <span v-else class="font-bold">{{ order._attributes.collectionByUserName }}</span>
                </div>
            ...
            <p class="flex items-center space-x-2 text-xs">
                ...
                <LineSkeleton v-if="isLoadingOrder" width="w-24"></LineSkeleton>
                <span v-else class="font-bold">{{ order._attributes.collectionVerifiedByUserName }}</span>
            </p>
...
<script>
    ...
    export default {
        ...
        components: { Alert, Checkbox, ShineEffect, OtpInput, LineSkeleton, PrimaryButton, RoundSkeleton, MoreInfoPopover, BadgeIndicator },
        props: {
            order: {
                type: Object
            },
