<template>
    <div>
        ...
        <div class="flex justify-end space-x-2 p-4 border rounded-lg bg-white">
            <Pill type="primary" size="xs" :showDot="false" :action="showBulkAddTimeslotsModal">+ Bulk Add Timeslots</Pill>
            <Pill type="primary" size="xs" :showDot="false" :action="showBulkResetTimeslotsModal">Reset Timeslots</Pill>
        </div>
            ...
            <div v-if="operationalHour.available" class="col-span-1 space-y-2">
                ...
                <div v-for="(hour, index2) in operationalHour.hours" :key="index2">
                    ...
                    <div class="flex items-center space-x-2">
                        ...
                        <TimeInput v-model="form.operationalHours[index].hours[index2][0]" :errorText="formState.getFormError('operationalHours'+index+'Hours'+index2+'0')" class="w-24"></TimeInput>
                        <span>-</span>
                        <TimeInput v-model="form.operationalHours[index].hours[index2][1]" :errorText="formState.getFormError('operationalHours'+index+'Hours'+index2+'1')" class="w-24"></TimeInput>
...
<script>
    ...
    export default {
        ...
        components: {
            Pill, TimeInput, Checkbox, ConfirmModal, MoreInfoPopover
        },
        props: {
            form: {
                type: Object
        ...
        watch: {
            'form.autoGenerateTimeSlots'(newValue, oldValue) {
                ...
                if(newValue) {
                    ...
                    this.notificationState.showSuccessNotification(`Added ${totalDiff} ${totalDiff == 1 ? 'timeslot' : 'timeslots'}`);
        ...
        methods: {
            ...
            bulkResetTimeslots(hideModal) {
                ...
                this.notificationState.showSuccessNotification('Timeslots reset');
